            ...
            <div class="grid lg:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center justify-between">
                        ...
                        <span class="text-[10px] font-bold text-gray-400 uppercase bg-gray-100 px-2 py-0.5 rounded-full">Base: Fijo</span>
                    </h3>

                    <div class="space-y-4">
                        {{-- Main Indicator --}}
                        <div class="p-4 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl border border-emerald-100 relative overflow-hidden">
                            <p class="text-xs font-bold text-emerald-600 uppercase tracking-wider mb-1 relative z-10">Promedio Real Mensual</p>
                            ...
                            <div class="mt-4 relative pt-5">
                                ...
                                <div class="h-2 w-full bg-gray-200 rounded-full relative">
                                    ...
                                    {{-- Markers --}}
                                    <div class="absolute -top-5 text-[9px] font-bold text-gray-400" style="left: {{ $fixedPos }}%; transform: translateX(-50%)">
                                        FIJO
                                    </div>
                                    <div class="absolute h-4 w-0.5 bg-gray-400 top-0" style="left: {{ $fixedPos }}%"></div>
                                    
                                    @if($configuredIncome > $configuredFixedIncome)
                                    <div class="absolute -top-5 text-[9px] font-bold text-emerald-500" style="left: {{ $totalPos }}%; transform: translateX(-50%)">
                                        +SPORAD.
                                    </div>
                        ...
                        @if($incomeDiff != 0)
                        <div class="flex items-center gap-2 p-2 px-3 rounded-lg {{ $incomeDiff >= 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700' }} text-[11px] font-semibold">
                            <span>{{ $incomeDiff >= 0 ? '▲' : '▼' }} {{ abs($incomeDiffPercent) }}%</span>
                            <span class="opacity-70">{{ $incomeDiff >= 0 ? 'sobre tu ingreso fijo base.' : 'por debajo del ingreso fijo.' }}</span>
                        </div>
                        @endif

                        <p class="text-[10px] text-gray-400 italic text-center px-4">
                            Los ingresos esporádicos (bonos, aguinaldos) ayudan a cubrir gastos, pero tu base está en los ${{ number_format($configuredFixedIncome, 0) }}.
                        </p>
                    </div>
                </div>
